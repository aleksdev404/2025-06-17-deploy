...
def totals(year: int | None = Query(None, ge=2000),
    ...
    q = q.filter(models.StockMovement.created_at >= start, models.StockMovement.created_at < end)

    rows = (q.group_by(models.Material.name).order_by(func.sum(models.StockMovement.qty)).all())
